...
- (BOOL)canBecomeFirstResponder {
    NSLog(@"测试canBecomeFirstResponder");
    return YES;
}
...
-(void)centralManagerDidUpdateState:(CBCentralManager *)central{
    ...
    switch (central.state) {
        case CBCentralManagerStateUnknown:
            ...
            error = @"CBCentralManagerStateUnknown";
            break;
        case CBCentralManagerStateResetting:
            NSLog(@">>>CBCentralManagerStateResetting");
            error = @"CBCentralManagerStateResetting";
            break;
        case CBCentralManagerStateUnsupported:
            NSLog(@">>>CBCentralManagerStateUnsupported");
            error = @"CBCentralManagerStateUnsupported";
            break;
        case CBCentralManagerStateUnauthorized:
            NSLog(@">>>CBCentralManagerStateUnauthorized");
            error = @"CBCentralManagerStateUnauthorized";
            break;
        case CBCentralManagerStatePoweredOff:
            //[self.commandDelegate sendPluginResult:@"请开启蓝牙" callbackId:command.callbackId];
            NSLog(@">>>CBCentralManagerStatePoweredOff");
            error = @"CBCentralManagerStatePoweredOff";
            break;
        case CBCentralManagerStatePoweredOn:
            error = @"CBCentralManagerStatePoweredOn";
...
-(void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error{
    ...
    for (CBService *service in peripheral.services) {
        NSLog(@"蓝牙设备的UUID值：%@,属性值：%@",service.UUID,service.description);
...
-(void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error{
    ...
    for (CBCharacteristic *characteristic in service.characteristics)
    {
        ...
        NSLog(@"服务service:%@ 的 特征Characteristic: %@ 的值：%@",service.UUID,characteristic.UUID, characteristic.observationInfo);
    }
...
-(void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    ...
    NSLog(@"获取特征值characteristic uuid:%@  value:%@",characteristic.UUID,characteristic.value);
...
- (void)didReceiveMemoryWarning {
    NSLog(@"警告：didReceiveMemoryWarning");
...
//监听读特征的协议函数
-(void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic :(NSError *)error
{
